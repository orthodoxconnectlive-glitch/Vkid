...
export const CreateEventModal: React.FC<CreateEventModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const created = await saveEvent({
      ...
      category,
      parish: profile?.parish || 'Holy Trinity Cathedral',
      hostName: profile?.full_name || 'Fr. Seraphim Rose',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-xl bg-[#fdfaf5] border border-[#d4af37]/30 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        <div className="p-5 border-b border-[#d4af37]/20 bg-[#f1ebd7] flex items-center justify-between">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
          <div>
            <label className="block text-xs font-bold text-[#8b6b4a] uppercase tracking-wider mb-1.5">
              Event Title *
            </label>
            <input
              type="text"
              required
              placeholder="e.g. Hierarchical Divine Liturgy & Panagia Blessing"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full px-3.5 py-2.5 rounded-xl bg-[#f5f2ed] border border-[#d4af37]/30 text-xs text-[#2c2c2c] placeholder-[#8b6b4a]/60 focus:outline-none focus:border-[#d4af37]"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-[#f5f2ed] border border-[#d4af37]/30 text-xs text-[#2c2c2c] focus:outline-none focus:border-[#d4af37]"
              >
                <option value="liturgy">Divine Liturgy & Service</option>
                <option value="feast">Great Feast Day</option>
                <option value="bible_study">Bible Study & Patristics</option>
                <option value="youth">Youth Fellowship</option>
                <option value="pilgrimage">Pilgrimage & Retreat</option>
                <option value="choir">Choir Rehearsal</option>
                <option value="social">Parish Social & Festival</option>
              </select>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-[#f5f2ed] border border-[#d4af37]/30 text-xs text-[#5a4632] font-semibold">
                <Church className="w-4 h-4 text-[#d4af37]" />
                <span>{profile?.parish || 'Holy Trinity Cathedral'}</span>
              </div>
            </div>
          </div>
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border transition-all cursor-pointer ${
                  locationType === 'physical'
                    ...
                    : 'bg-[#f5f2ed] text-[#8b6b4a] border-[#d4af37]/20 hover:border-[#d4af37]/40'
                }`}
              >
          ...
          {locationType === 'physical' ? (
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Holy Trinity Cathedral Hall, 450 Harvard St, Boston"
            ...
            <div>
              <label className="block text-xs font-bold text-[#8b6b4a] uppercase tracking-wider mb-1.5">
                Virtual Link or Stream URL
              </label>
              <input
                type="url"
                placeholder="https://orthodoxconnect.live/room-bible or Zoom link"
                value={virtualLink}
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder="Describe the service, feast commemoration, fasting guidelines, or schedule..."
              value={description}
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-[#f5f2ed] border border-[#d4af37]/30 text-xs text-[#2c2c2c] focus:outline-none focus:border-[#d4af37] mb-2"
            />
            <div className="flex gap-2">
          ...
          <div className="pt-3 border-t border-[#d4af37]/20 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 rounded-xl text-xs font-bold text-[#8b6b4a] hover:bg-[#f1ebd7] transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
