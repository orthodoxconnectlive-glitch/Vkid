...
export const VideoPlayerModal: React.FC<VideoPlayerModalProps> = ({
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-3xl overflow-hidden max-w-3xl w-full border-4 border-amber-300 shadow-2xl relative my-auto max-h-[94vh] flex flex-col">
          ...
          <div className="p-3.5 bg-gradient-to-r from-amber-400 via-orange-400 to-amber-500 text-slate-900 flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {/* External Link protected by Parental Gate */}
              <button
                type="button"
                tabIndex={0}
                ...
                title="Open video directly (Parental Verification Required)"
              >
            ...
            {currentMedia.type === 'video' || currentMedia.type === 'rhyme' ? (
              <div className="aspect-video w-full rounded-2xl overflow-hidden bg-black shadow-2xl">
                <TvVideoPlayer
                  ...
                  onOpenExternal={handleExternalLinkClick}
                />
              </div>
      ...
      {/* Parental Gate Modal for Outbound External Link */}
      {isParentGateOpen && (
        <ParentGateModal
          title="Parental Verification"
          reason="external_link"
          targetUrl={targetWatchUrl}
          description="To protect children, a parent or guardian must verify their age before leaving VKid to open an external video link."
          onSuccess={() => {
            setIsParentGateOpen(false);
          }}
