...
export const GroupRoomModal: React.FC<GroupRoomModalProps> = ({ room, isOpen, onClose }) => {
  ...
  const [chatList, setChatList] = useState<RoomChatMessage[]>([
    { sender: 'Fr. Seraphim', text: 'Welcome brothers and sisters to tonight’s Bible Study!', time: '20:01' },
    { sender: 'Eleni C.', text: 'Glory to God! Present with choir members.', time: '20:02' },
  ]);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setChatList((prev) => [
      ...prev,
      {
        sender: profile?.full_name || 'Parishioner',
  ...
  // Mock participants
  const participants = [
    { name: room.hostName, role: 'Host', avatar: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&q=80&w=200' },
    { name: profile?.full_name || 'You', role: 'Participant', avatar: profile?.avatar_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=200' },
    { name: 'Deacon Markos', role: 'Co-Host', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=200' },
    { name: 'Anna Papadopoulos', role: 'Participant', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=200' },
  ];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-4xl bg-stone-950 border border-amber-600/40 rounded-2xl p-6 shadow-2xl text-stone-100 flex flex-col md:flex-row gap-6 max-h-[90vh]">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-1.5 rounded-full text-stone-400 hover:text-amber-300 hover:bg-stone-900 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
          ...
          <div className="flex-1 grid grid-cols-2 gap-3 min-h-[260px] bg-stone-900/90 rounded-2xl p-3 border border-amber-900/30 overflow-y-auto">
            {participants.map((p, idx) => (
              ...
              >
                ...
                {idx === 1 && isHandRaised && (
                  <span className="absolute top-2 right-2 p-1 rounded-full bg-amber-500 text-stone-950 shadow-md">
        ...
        <div className="w-full md:w-80 flex flex-col border-t md:border-t-0 md:border-l border-amber-900/40 pt-4 md:pt-0 md:pl-4 min-h-[300px]">
          <h4 className="font-serif font-bold text-xs text-amber-300 mb-3 flex items-center gap-2">
            <MessageSquare className="w-4 h-4 text-amber-400" /> Room Fellowship Chat
          </h4>
